...
app.post('/api/Adminlogin', (req,resp)=>{
    const receivedData = req.body
    console.log(receivedData)
    con.query('select * from admins where ID = ? and pass = md5(?)',[receivedData.name, receivedData.password],(error, result)=>{
        if(error){
            console.log(error)
        }else{
            console.log(result)
...
app.post('/api/addIndicustomer', (req,resp)=>{
    const receivedData = req.body
    con.query('INSERT INTO indi_customer(NIC, FName, LName, custID) values (?,?,?,?)',
...
app.post('/api/addPhoneNo', (req,resp)=>{
    const receivedData = req.body
    con.query('INSERT INTO PhoneNo_customer(custID, Phone) values (?,?)',[receivedData.custID, receivedData.Phone], (error, result)=>{
        if(error){
            console.log(error)
        }else{
            console.log(result)
    ...
    if(receivedData.Alternate){
        con.query('INSERT INTO PhoneNo_customer(custID, Phone) values (?,?)',[receivedData.custID, receivedData.Alternate], (error, result)=>{
            if(error){
                console.log(error)
            }else{
                console.log(result)
...
app.post('/api/addCompPhoneNo', (req,resp)=>{
    ...
    if(receivedData.Fax){
        con.query('INSERT INTO PhoneNo_customer(custID, Phone) values (?,?)',[receivedData.custID, receivedData.Fax], (error, result)=>{
            if(error){
                console.log(error)
            }else{
                console.log(result)
